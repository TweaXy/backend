/*
  ...
  - The primary key for the `trendsinteractions` table will be changed. If it partially fails, the table could be left without primary key constraint.
  - You are about to drop the column `trendID` on the `trendsinteractions` table. All the data in the column will be lost.
  ...
  - Added the required column `trend` to the `TrendsInteractions` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `trendsinteractions` DROP PRIMARY KEY,
    DROP COLUMN `trendID`,
