...
test('sucessfully edit profile info', async () => {
    ...
    const response =await supertest(app).patch('/api/v1/users/')
...
test('sucessfully edit profile picture ', async () => {
    ...
    const response1=await supertest(app).patch('/api/v1/users/')
        .set({ Authorization: `Bearer ${token}` })
        ...
        const response2=await supertest(app).delete('/api/v1/users/profilePicture')
        .set({ Authorization: `Bearer ${token}` })
        .expect(200);
...
test('sucessfully edit profile Banner ', async () => {
    ...
    const response1=await supertest(app).patch('/api/v1/users/')
        ...
        const response2=await supertest(app).delete('/api/v1/users/profileBanner')
        .set({ Authorization: `Bearer ${token}` })
        .expect(200);
