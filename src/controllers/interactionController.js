...
const deleteinteraction = catchAsync(async (req, res, next) => {
    ...
    const checkInteractions=await intercationServices.checkInteractions(req.params.id);
    ...
    const checkUserInteractions=await intercationServices.checkUserInteractions(req.user.id,req.params.id);
    if(!checkUserInteractions)
    {
        return next(new AppError('user not authorized',401));
    }
    await intercationServices.deleteinteraction(req.params.id);
